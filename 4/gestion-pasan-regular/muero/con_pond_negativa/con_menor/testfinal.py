...
#    for ponderation in ponds:
    for ponderation,  peso in zip(ponds,  pesos):
        ponderation = ponderation[:-1]
        peso = peso[:-1]
        results += ponderation
        results = results + '\t' +  peso 
        results += '\t'
        res = subprocess.run(args = ['./testfinal.sh', ponderation,  peso], stdout=subprocess.PIPE)
         
        
        NUMERO_BOTS = 1004
        out1 = get_out('jugador2.cl')
