...
def main():
    ...
    outputName = 'pond_Rodri_neg_alto_' + sys.argv[1]
    ...
    random.seed()
    while 1:    
        perm = [str(30*random.randint(-10, 10)) for i in range(12)]
        ...
        for i in range(6, 11):
            pond = pond + perm[i] + ' '
        pond = pond + perm[11] + ')))'
        peso = '(defvar *peso* ' + perm[12] + ')\t\t\t\t\t\t'

        #Llamamos al script y, si pasa al regular, guardamos las ponderaciones        
        res = subprocess.run(['./pasaregular8.sh', pond, peso], stdout = subprocess.PIPE)
        ...
        if win_rate1 > 0.75:
            results = pond + '\t' + peso + '\t' + str(win_rate1) + '\n'
            if win_rate1 > 0.85:
