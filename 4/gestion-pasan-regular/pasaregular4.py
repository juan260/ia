...
#    resultsKK = ''
    ...
    for perm in perms:
        ...
        args = '(defvar *ponderations* \'(('+ str(perm[0]) + ' ' + str(perm[1]) + ' ' + str(perm[2]) + ' ' + str(perm[3]) + ' 0 0)(' + str(perm[4]) + ' ' + str(perm[5]) + ' ' + str(perm[6]) + ' ' + str(perm[7]) + ' 0 0)))\n'          
        #Llamamos al script y, si pasa al regular, guardamos las ponderaciones        
        res = subprocess.run(['./CreateAndExecute.sh', str(perm[0]), str(perm[1]), str(perm[2]), str(perm[3]), '0', '0', str(perm[4]), str(perm[5]), str(perm[6]), str(perm[7]), '0', '0'], stdout=subprocess.PIPE)
